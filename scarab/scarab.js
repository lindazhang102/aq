...
var ScarabUtil = function () {
    ...
    /**
     * 异步加载js
     * @param module 模块名，这玩意儿其实是个资源表的key
     * @param cb
     * @param hashConfig
     */
    var loader = function (module, cb, hashConfig) {
        ...
        /**
         * 保证回调函数只执行一次，兼容ie和w3c
         * @TODO callback传的参数很可能是null?
         * @param module
         */
...
var ScarabModule = function (win, config) {
    /**
     * cookie 的key映射
     * @type {{VIEWCOOKIE: string, ADDCOOKIE: string, VISITORCOOKIE: string, PROFILECOOKIE: string}}
     */
    var STRINGS = {
        ...
        /**
         *
         * @type {{v: string, ai: string, ca: string, co: string, k: string, q: string, vc: string, cp: string, lang: string, az: string}}
         */
        SERIALIZE = {
        ...
        /**
         * 配置特性检测
         * @type {{RELATED: {trigger: string, validate: FEATURETRIGGERS.RELATED.validate}, ALSO_BOUGHT: {trigger: string, validate: FEATURETRIGGERS.ALSO_BOUGHT.validate}, CART: {trigger: string, validate: FEATURETRIGGERS.CART.validate}}}
         */
        FEATURETRIGGERS = {
            'RELATED': {
                trigger: 'view',
            ...
            for (var key in conf) {
                if (key === 'orderId') {
                    ...
                    for (var i = 0, l = conf.items.length; i < l; i++) {
                        if (!isValid(tr, 'purchase', conf.items[i].item, 'item', ['string', 'number']) || !isValid(tr, 'purchase', conf.items[i].price, 'price', 'number', ['string', 'undefined', 'null']) || !isValid(tr, 'purchase', conf.items[i].quantity, 'quantity', 'number', ['string', 'undefined', 'null'])) {
                            return;
                        }
